...
<script>$(document).ready(function () {
        let uuid = '<?php echo $_GET['uuid'] ?>';


        $("#data-card-content").load('../inc/php/dep/pages/player.php?uuid=' + uuid, function (data) {
            console.log('loaded');
        });

        $("#data-stats-content").load('../inc/php/dep/pages/playerdata.php?uuid=' + uuid, function (data) {
            console.log('loaded');
        });

        $("#data-sessions-content").load('../inc/php/dep/pages/playersessions.php?p=1&uuid=' + uuid, function (data) {
            console.log('loaded');
        });

        $("#data-notes-content").load('../inc/php/dep/pages/playernotes.php?uuid=' + uuid, function (data) {
            console.log('loaded');
        });

        $("#data-punishments-content").load('../inc/php/dep/pages/playerpunishments.php?uuid=' + uuid, function (data) {
            console.log('loaded');
        });
        ...
        $(document).on('click', 'a', function () {
            if (this.hasAttribute('playersessions')) {
                ...
                if (typeof attr !== typeof undefined && attr !== false) {
                    $("#data-sessions-content").load('../inc/php/dep/pages/playersessions.php?uuid=' + uuid + '&p=' + attr, function () {
        ...
        $(document).on('click', 'button', function () {
            if (this.hasAttribute('nmdelete-playernote')) {
                let attr = $(this).attr('nmdelete-playernote');
                swal({
                    title: "Are you sure?",
