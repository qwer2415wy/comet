...
if (!isset($_GET['load'])) {
        ...
        foreach ($iterator as $row) {
            ...
            if (hasPermission('edit_tags')) {
                echo '<td class="text-right">
                                     <div class="fb-table-cell-wrapper">
                                            <button style="background: transparent; border: none" nmedit-tag="' . $row['id'] . '" style="position: relative;right: -24px;"><i class="material-icons" style="color: orange">create</i></button>
                                            <button style="background: transparent; border: none" nmdelete-tag="' . $row['id'] . '" style="position: relative;right: -24px;"><i class="material-icons" style="color: red">delete</i></button>
                                     </div>
                                </td>';
            }
                            echo '</tr>';
    ...
    $tag = html_entity_decode($purifier->purify($_GET['tag']));
    $description = html_entity_decode($purifier->purify($_GET['description']));
    ...
    die($result);
} else if ($_GET['load'] == 'search') {
    $search = $_GET['q'];
    $query = '%' . $search . '%';

    $total = $pdo->prepare('SELECT COUNT(*) FROM nm_tags WHERE (id LIKE ? || name LIKE ? || tag LIKE ?)');
    ...
    $start = $offset + 1;
    $end = min(($offset + $limit), $total);
    $prevlink = ($page > 1) ? '<a nmsearchtags="' . ($page - 1) . '" class="nm-pagination-icon"  page="' . ($page - 1) . '"><i class="material-icons">keyboard_arrow_left</i></a>' : '';
    $nextlink = ($page < $pages) ? '<a nmsearchtags="' . ($page - 1) . '" class="nm-pagination-icon" page="' . ($page + 1) . '"><i class="material-icons">keyboard_arrow_right</i></a>' : '';
    $stmnt = $pdo->prepare('SELECT id, name, tag, description, server FROM nm_tags WHERE (id LIKE ? || name LIKE ? || tag LIKE ?) ORDER BY id ASC LIMIT ? OFFSET ?');
    $stmnt->bindParam(1, $query, 2);
    $stmnt->bindParam(2, $query, 2);
    $stmnt->bindParam(3, $query, 2);
    $stmnt->bindParam(4, $limit, PDO::PARAM_INT);
    $stmnt->bindParam(5, $offset, PDO::PARAM_INT);
    $stmnt->execute();
    if ($stmnt->rowCount() > 0) {
        $stmnt->setFetchMode(PDO::FETCH_ASSOC);
        $iterator = new IteratorIterator($stmnt);
        echo '<div class="data-card-body">
                        <table class="fb-table-elem">
                            <thead>
                            <tr>
