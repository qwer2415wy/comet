...
class BruteForceBlock
{
    ...
    public static function addFailedLoginAttempt($user_id, $ip_address)
    {
        ...
        try {
            $stmt = $pdo->query('INSERT INTO nm_web_failed_logins SET user_id= ' . $user_id . ', ip_address = INET_ATON("' . $ip_address . '"), attempted_at = NOW()');
    ...
    public static function getLoginStatus($options = null)
    {
        ...
        try {
            $stmt = $pdo->query('SELECT MAX(attempted_at) AS attempted_at FROM nm_web_failed_logins');
            $row = $stmt->fetch();
        ...
        try {
            ...
            $get_number = $pdo->query('SELECT * FROM nm_web_failed_logins WHERE attempted_at > DATE_SUB(NOW(), INTERVAL ' . self::$time_frame_minutes . ' MINUTE)');
            ...
            /*if(self::$_db['auto_clear'] == true){
                ...
                try{
                    ...
                    $stmt = $pdo->query('DELETE from user_failed_logins WHERE attempted_at < DATE_SUB(NOW(), INTERVAL '.(self::$time_frame_minutes * 2).' MINUTE)');
                    $stmt->execute();
    ...
    public static function clearDatabase()
    {
        ...
        try {
            $pdo->query('DELETE FROM nm_web_failed_logins');
            return true;
        } catch (PDOException $ex) {
            //return errors
            return $ex;
