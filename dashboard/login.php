...
$jsondata = file_get_contents('../protected/settings.json');
...
if (isset($_COOKIE['nm_username'])) {
    ...
    if (!empty($username)) {
        $sql = 'SELECT `usergroup` FROM nm_accounts WHERE `username`=? AND `password`=?;';
        ...
        if (count($result) > 0) {
            ...
            foreach ($pdo->query('SELECT `COLUMN_NAME` FROM `INFORMATION_SCHEMA`.`COLUMNS` WHERE `TABLE_NAME`="nm_accountgroups"') as $item) {
                ...
                $stmnt2->bindParam(1, $usergroup, 2);
                $stmnt2->execute();
                $res = $stmnt2->fetchAll();
                if ($res[0][0] == '1') {
            ...
            $group = new Group($usergroup, $permissions);
            $_SESSION['user'] = new User($username, $result[0][2], $group);
            header('Location: dashboard.php');
        }
    }
}
    ...
    $("#login").submit(function (event) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response === "true") {
                ...
                } else if (response.startsWith('delay')) {
                    document.getElementById("title").innerHTML = "Locked out. Wait " + response.replace('delay ', '') + 'seconds';
                    document.getElementById("header").style.cssText = 'background-color:red !important';
                    document.getElementById("username").value = "";
                    document.getElementById("password").value = "";
                    ...
                    document.getElementById("title").innerHTML = "You are now locked out.";
                    document.getElementById("header").style.cssText = 'background-color:red !important';
                    document.getElementById("username").value = "";
                    document.getElementById("password").value = "";
